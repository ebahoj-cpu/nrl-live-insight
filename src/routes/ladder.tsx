...
export const Route = createFileRoute("/ladder")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Live NRL standings: wins, losses, points differential, and competition points." },
      { property: "og:title", content: "NRL Ladder — LINEBREAK" },
      { property: "og:description", content: "Live NRL standings, top 8, and points differential." },
    ],
  }),
...
function LadderPage() {
  return (
    <Suspense fallback={<div className="pt-8"><div className="h-8 w-48 bg-surface rounded mb-4 animate-pulse" /><div className="h-96 bg-surface rounded animate-pulse" /></div>}>
      <Ladder />
    </Suspense>
  );
}
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr
                    key={r.teamId}
                    className={`border-t border-border ${inTop8 ? "bg-accent/5" : ""}`}
                  >
