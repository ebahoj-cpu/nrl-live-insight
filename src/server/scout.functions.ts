...
export const scoutChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ reply: string }> => {
    ...
    const system = [
      ...
      "• Reply with 5–8 markdown bullets only. Each bullet starts with '- '.",
      "• Each bullet MUST follow this exact shape:",
      "    - **PICK / MARKET** `@PRICE` — sharp reason (≤14 words)",
      "  Examples:",
      ...
      "    - **Dolphins H2H** `@1.68` — Storm 0-5 last five, Hughes out leaves playmaking void",
      "• Wrap the pick/market in **bold**. Wrap the price in `backticks` so it renders as code.",
      "• Use ' — ' (em dash with spaces) between price and reason. No other separators.",
      "• Max 20 words per bullet total. No filler, no hedging, no journalism.",
      "• NO intro line. NO summary line. NO disclaimers. NO headings. Bullets only.",
      "",
      "FOCUS — pick the angles that fit the question:",
      "edge mismatches · possession & territory · game script · fatigue windows · spine threats · price vs line value.",
      "",
      "DATA RULES:",
      "• Quote exact prices/lines from SNAPSHOT. If price missing, omit the `@PRICE` segment but keep the bullet shape.",
      "• Never invent prices, players, or stats. 'Model estimate' = fallback, not live book.",
      "",
      "=== SNAPSHOT ===",
      context,
      "=== END SNAPSHOT ===",
